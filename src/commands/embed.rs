...
use serenity::{Embed, ExecuteWebhook, Webhook};

use crate::{error::respond_error, util::embeds::EmbedColor, Context};
...
pub async fn simple(
        ...
        .await
    {
        ...
        Err(e) => {
            respond_error("Failed to post embed in {channel} channel", e, &ctx).await?;
        }
    }
...
/// Pass in a json object to cu
#[poise::command(slash_command, guild_only = true)]
pub async fn custom(
    ctx: Context<'_>,
    #[description = "The target channel webhook for your embed to go."]
    target: TargetChannelWebhook,
